...
const files = new Map();
files.set('/fixture.js',resolve(base,'../u4-2b-integrated-junction-v1/fixture.js'));
files.set('/static/style.css',resolve(staticRoot,'style.css'));
files.set('/static/index.shell-open.html',resolve(staticRoot,'index.shell-open.html'));
for (const file of ['index.html','composition.css','composition.js']) files.set(file === 'index.html' ? '/' : `/${file}`, resolve(base,file));
...
const server = createServer(async (req,res) => {
  ...
  const path = url.pathname === '/image-cache' && url.searchParams.get('url') === 'demo:metropolis'
    ? resolve(base,'../u4-2b-integrated-junction-v1/metropolis-poster.jpg') : files.get(url.pathname);
