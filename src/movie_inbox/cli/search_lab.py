...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Measure the current search behavior without writing to a catalog or using the network."
        )
    )
    commands = parser.add_subparsers(dest="command", required=True)

    run_parser = commands.add_parser(
        "run", help="Run the packaged golden corpus or a custom corpus."
    )
    ...
    inspect_parser.add_argument(
        "catalog", type=Path, help="JSON catalog export to inspect read-only."
    )
    ...
    inspect_parser.add_argument(
        "--year", default="", help="Candidate year for identity or scanner mode."
    )
    inspect_parser.add_argument(
        "--kind", default="pelicula", help="Candidate kind for identity or scanner mode."
    )
...
def add_report_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--html", dest="html_report", type=Path, help="Write a standalone HTML report."
    )
...
def print_inspection_summary(report: Mapping[str, Any]) -> None:
    ...
    for index, row in enumerate(report["results"][:10], 1):
        ...
        line = f"  {index:>2}. {title}{year} score={row.get('score', 0)} "
        line += f"{row.get('reason', '')}{accepted}"
        print(line.rstrip())
...
<head>
  ...
  <style>
    :root {{
      color-scheme: dark; --bg:#090a18; --panel:#12152a; --line:#343960; --text:#f4f1ff;
      --muted:#adb3cf; --cyan:#35d7e8; --pink:#ff45bd; --good:#67dfa5; --bad:#ff7d96;
    }}
    * {{ box-sizing:border-box; }}
    ...
    h1 {{ margin:0 0 8px; font-size:clamp(28px,5vw,52px); letter-spacing:0; }}
    h2 {{ font-size:19px; margin:0; }}
    p {{ color:var(--muted); }}
    .eyebrow {{ color:var(--cyan); font-size:12px; font-weight:800; text-transform:uppercase; }}
    .metrics {{
      display:grid; grid-template-columns:repeat(auto-fit,minmax(150px,1fr)); gap:1px;
      margin:24px 0; border:1px solid var(--line); background:var(--line);
    }}
    .metric {{ min-height:96px; padding:16px; background:var(--panel); }}
    .metric strong {{ display:block; font-size:25px; }}
    .metric span {{ color:var(--muted); }}
    .case {{ margin:14px 0; border:1px solid var(--line); background:var(--panel); }}
    .case summary {{ cursor:pointer; padding:14px 16px; font-weight:750; }}
    ...
    .pill {{
      display:inline-block; border:1px solid var(--line); padding:2px 7px; margin:2px;
      font-size:12px;
    }}
...
<body><main>
<div class="eyebrow">Movie Inbox / lectura solamente</div>
<h1>{_escape(title)}</h1>
<p>Ranking productivo actual, ejecutado sin red y sin escribir en el catálogo.</p>
{summary}{cases}
</main></body>
</html>
"""
...
def _corpus_summary_html(report: Mapping[str, Any]) -> str:
    ...
    checks = "".join(
        f'<span class="pill {"pass" if row.get("passed") else "fail"}">'
        f"{_escape(row.get('metric'))}: {_escape(row.get('actual'))} / "
        f"{_escape(row.get('target'))}</span>"
...
def _case_html(case: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        f"{_escape(case.get('label'))} <code>{_escape(case.get('context'))}</code></summary>"
        '<div class="case-body">'
        f"<p>Consulta: <code>{_escape(case.get('query'))}</code> "
        f"· P@K {_number(metrics.get('precision_at_k'))} · "
        f"R@K {_number(metrics.get('recall_at_k'))} · "
        f"RR {_number(metrics.get('reciprocal_rank'))}</p>"
...
def _results_table(results: Sequence[Mapping[str, Any]]) -> str:
    ...
    rows = "".join(
        ...
        f"<td>{'sí' if row.get('accepted') else 'no'}</td>"
        f"<td><code>{_escape(row.get('key'))}</code></td>"
        "</tr>"
        for index, row in enumerate(results, 1)
    )
    return (
        '<div class="table-wrap"><table><thead><tr>'
        "<th>#</th><th>Título</th><th>Año</th><th>Score</th>"
